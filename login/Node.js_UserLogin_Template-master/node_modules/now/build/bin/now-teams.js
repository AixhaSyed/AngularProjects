...
if (argv.help || !subcommand) {
  ...
  Promise.resolve().then(() => __async(function*(){
    ...
    let token
    try {
      token = argv.token || config.token || ((yield login(apiUrl)))
    } catch (err) {
...
function run({ token, config: { currentTeam } }) {return __async(function*(){
  ...
  switch (subcommand) {
    case 'switch':
    case 'change': {
      yield require(resolve(__dirname, 'teams', 'switch.js'))(teams, args)
      break
    }
    case 'add':
    case 'create': {
      yield require(resolve(__dirname, 'teams', 'add.js'))(teams)
      break
    }

    case 'invite': {
      yield require(resolve(__dirname, 'teams', 'invite.js'))(teams, args)
      break
    }
